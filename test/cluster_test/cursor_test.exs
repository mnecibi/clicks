defmodule Clicks.CursorTest do
  use Clicks.DataCase

  alias Clicks.Cursor

  describe "cursor_positions" do
    alias Clicks.Cursor.CursorPositions
    ...
    test "create_cursor_positions/1 with valid data creates a cursor_positions" do
      ...
      assert {:ok, %CursorPositions{} = cursor_positions} =
               Cursor.create_cursor_positions(valid_attrs)
    ...
    test "update_cursor_positions/2 with valid data updates the cursor_positions" do
      ...
      assert {:ok, %CursorPositions{} = cursor_positions} =
               Cursor.update_cursor_positions(cursor_positions, update_attrs)
    ...
    test "update_cursor_positions/2 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} =
               Cursor.update_cursor_positions(cursor_positions, @invalid_attrs)
    ...
    test "delete_cursor_positions/1 deletes the cursor_positions" do
      ...
      assert_raise Ecto.NoResultsError, fn ->
        Cursor.get_cursor_positions!(cursor_positions.id)
      end
    end
